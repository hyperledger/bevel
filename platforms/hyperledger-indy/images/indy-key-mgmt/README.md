...
```bash
docker build -t <docker_url>/indy-key-mgmt:1.9.2 .
```
...
```bash
docker build --build-arg INDY_NODE_VERSION=v1.11.0 -t <docker_url>/indy-key-mgmt:1.11.0 .
```
...
### Example:
```bash
docker run -it --rm -e VAULT_TOKEN=<your_token> <docker_url>/indy-key-mgmt:1.9.2 generate_identity <your_identity_name> <your_vault_path> <your_target> http://<your_vault_address>:8200
```

Insert to vault:
```bash
docker run -it --rm -e VAULT_TOKEN="s.ev8ehHRFYgluTkVDYFH7X5vE" <docker_url>/indy-key-mgmt:1.9.2 generate_identity my-identity provider.stewards vault http://host.docker.internal:8200
```

Print on console:
```bash
docker run -it --rm <docker_url>/indy-key-mgmt:1.9.2 bash -c "generate_identity my-identity provider.stewards | jq"
```
