...
if [ "$NETWORK_VERSION" != "2.5.4" ]; then
    ...
    echo "version 2.5.4++++"
    jq --argjson a "$(cat ./orderer)" '.channel_group.groups.Orderer.groups.{{ component_name }}MSP.values.Endpoints.value.addresses += $a' {{ channel_name }}_config.json > {{ channel_name }}_modified_intermediate_endpoints_config.json
