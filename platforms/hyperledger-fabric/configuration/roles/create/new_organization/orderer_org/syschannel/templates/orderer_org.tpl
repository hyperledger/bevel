...
jq -s '.[0] * {"channel_group":{"groups":{"Consortiums":{"groups":{"{{ channel.consortium }}":{"groups": {"{{ component_name }}MSP":.[1]}}}}}}}' config1.json config.json > config2.json
cat config2.json | jq '.channel_group.groups.Orderer.values.ConsensusType.value.metadata.consenters += ['$(cat ./config_consenters.json)']' > modified_config.json
