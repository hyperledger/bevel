...
jq -s '.[0] * {"channel_group":{"groups":{"Orderer": {"values": {"Capabilities": { "value": .[1].orderer.value }}}}}}' {{ channel_name }}_config_block.json ./capabilities.json > modified_config.json
echo "copy current channel config and modified channel config"
