...
jq -s ".[0] * {\"channel_group\":{\"groups\":{\"Consortiums\":{\"groups\": {\"{{ consortium_name }}\": {\"groups\": {\"{{ org_name }}\": {\"policies\": .[1].{{ org_name }}Policies}}}}}}}}" {{ channel_name }}_config_block.json ./enable_endorsements_{{ org_name }}.json > modified_config.json
echo "copy the current channel config and modified channel config"
