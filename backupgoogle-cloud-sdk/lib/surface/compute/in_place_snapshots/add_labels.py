...
"""Command for adding labels to in-place snapshots."""
...
from googlecloudsdk.api_lib.compute import in_place_snapshots_utils as api_util
...
class AddLabels(base.UpdateCommand):
  """add-labels command for in-place snapshots."""
  IPS_ARG = None

  @classmethod
  def Args(cls, parser):
    cls.IPS_ARG = ips_flags.MakeInPlaceSnapshotArg()
  ...
  def Run(self, args):
    ...
    return waiter.WaitFor(
        ...
        'Updating labels of in-place snapshot [{0}]'.format(ips_ref.Name()))
