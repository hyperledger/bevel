  ...
  Fields:
    ...
    deniedPermissions: Specifies the permissions that are explicitly denied by
      ...
      * a full permission string in either of the following canonical formats
      ...
      string.   -- `*` represents all permissions.    Permission string can
      use more than one wildcard over multiple segments   in the permission
      string. However, it must match the entire segment and   no partial
      matches within a segment is allowed. For example,   --
      `compute.*.setIamPolicy` refers to all setIamPolicy permissions by any
      compute resources.   -- `iam.googleapis.com/*.create"` refers to all
      create permissions by any       iam resources.   --
      `compute.forwardingRules.*` refers to all permissions for forwarding
      rules.   -- `compute.*.*` refers to all permissions by all compute
      resource types.
    ...
    exceptionPrincipals: Specifies the identities requesting access for a
      ...
      `principal://goog/subject/alice@gmail.com`.  * Google and G Suite
      groups:   -- `principalSet://goog/group/{groupId}`: An identifier that
      represents a      Google group. For example,
      `principalSet://goog/group/admins@example.com`.  * Service Accounts:
      --   `principal://iam.googleapis.com/projects/-/serviceAccounts/{service
      AccountId}`:   An identifier that represents a service account. For
      example,   `principal://iam.googleapis.com/projects/-/serviceAccounts/sa
      123@iam.gserviceaccount.com`.  * G Suite Customers:   --
...
class GoogleIamV2alpha1Policy(_messages.Message):
  ...
  Fields:
    ...
    name: Immutable. The resource name of the `Policy`. Takes the form:
      `policies/{attachment-point}/{kind-plural}/{policy-id}`  For example, `p
      olicies/cloudresourcemanager.googleapis.com%2Fprojects%2F123/denypolicie
      s/a-deny-policy`.
