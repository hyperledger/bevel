...
---
For reference, sample `network.yaml` file looks like below for RAFT consensus (but always check the latest at `platforms/quourm/configuration/samples`):
...
network:
  ...
  config:    
    ...
    # tessera: 0.9.2 (for quorum 2.1.1)
    tm_version: "0.11"               
